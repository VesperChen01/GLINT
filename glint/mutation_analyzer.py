...
def _detect_foldx() -> Optional[str]:
    """
    检测 FoldX 可执行文件
    优先级：项目目录 external/foldx/ -> 环境变量/PATH
    """
    # 1. 优先检测项目目录
    try:
        project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        foldx_dir = os.path.join(project_root, 'external', 'foldx')

        if os.path.isdir(foldx_dir):
            # 支持多种 FoldX 可执行文件名
            candidates = [
                'foldx', 'foldx_20270131', 'foldx5', 'foldx_Mac_0',  # Mac/Linux
                'foldx.exe', 'foldx_1_20270131.exe', 'foldx5.exe'    # Windows
            ]

            for candidate in candidates:
                foldx_path = os.path.join(foldx_dir, candidate)
                if os.path.isfile(foldx_path) and os.access(foldx_path, os.X_OK):
                    return foldx_path
    except Exception:
        pass  # 如果项目目录检测失败，继续使用环境变量/PATH
...
def _pymol_mutate(obj_name: str, chain: str, resi: str, target_aa: str) -> bool:
    ...
    try:
        ...
        if cmd.count_atoms(selection) == 0:
            print(f"❌ Residue not found: chain {chain} resi {resi}")
            return False
        ...
        print(f"✅ Mutation succeeded: chain {chain} resi {resi} → {target_aa}")
        return True

    except Exception as e:
        print(f"❌ PyMOL mutation failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def minimize_energy(obj_name: str, selection: str = "all", cycles: int = 100,
    ...
    if method == "pymol":
        try:
            ...
            print(f"✅ PyMOL energy minimization completed: {cycles} iterations")
            return min_obj
        ...
        print("⚠️ Rosetta energy minimization is not implemented yet")
        return None

    else:
        print(f"❌ Unknown minimization method: {method}")
        return None
...
def perform_mutation(obj_name: str, mutations: List[Tuple[str, str, str]],
    ...
    if method == "pymol":
        ...
        print(f"✅ Completed {success_count}/{len(mutations)} mutations")
        return mut_obj

    elif method == "foldx":
        if not _detect_foldx():
            print("❌ FoldX was not detected")
            print("💡 Installation:")
            print("   1. Download FoldX: https://foldxsuite.crg.eu/")
            print("   2. Set the environment variable: export FOLDX=/path/to/foldx")
            cmd.delete(mut_obj)
            return None

        print("⚠️ For FoldX-based mutations, please use the ddg_heatmap command")
        print("💡 Example: ddg_heatmap('CRBN_sel', 'POI_sel')")
        cmd.delete(mut_obj)
        return None

    else:
        print(f"❌ Unknown mutation method: {method}")
        print("💡 Available methods: pymol, foldx")
        cmd.delete(mut_obj)
        return None
...
def calculate_mutation_ddg(wt_obj: str, mut_obj: str, partner_sel: Optional[str] = None,
    ...
    if method != "foldx":
        ...
        print("💡 Currently only the FoldX method is supported")
        return None
    ...
    if not _detect_foldx():
        ...
        print("   1. Download FoldX: https://foldxsuite.crg.eu/")
        print("   2. Set the environment variable: export FOLDX=/path/to/foldx")
        print("   Or add the FoldX executable to your PATH")
        return None

    print("💡 Tip: Use the ddg_heatmap command for a full ΔΔG analysis")
    print("   Example: ddg_heatmap('CRBN_sel', 'POI_sel')")
    return None
...
def ddg_heatmap(CRBN_sel: str, POI_sel: str, name: str = "ddg"):
    ...
    if not fx:
        ...
        print("[ddg_heatmap] 💡 Installation:")
        print("   1. Download FoldX: https://foldxsuite.crg.eu/")
        print("   2. Set the environment variable: export FOLDX=/path/to/foldx")
        print("   Or add the FoldX executable to your PATH")
        return
    ...
    try:
        # 准备 PDB
        print("[ddg_heatmap] 📦 Preparing complex PDB...")
        pdb_path, mapping = _prep_complex_tmp(crbn, poi)
        print(f"[ddg_heatmap] 📄 PDB exported: {pdb_path}")

        # 运行 FoldX 丙氨酸扫描
        print("[ddg_heatmap] 🧪 Running FoldX alanine scan (interface residues)...")
        ddg = _foldx_alanine_scan(fx, pdb_path, poi)

        if not ddg:
            print("[ddg_heatmap] ⚠️ No ΔΔG results were obtained (no interface residues detected or FoldX failed)")
            return

        # 设置 B 因子并着色
        set_b_factors(poi, ddg, default=0.0)
        color_by_b(poi, name=name)

        # 显示表面
        cmd.show("surface", poi)
        cmd.rebuild()

        print(f"[ddg_heatmap] ✅ Done: wrote ΔΔG to B-factors and colored {len(ddg)} residues")
    finally:
