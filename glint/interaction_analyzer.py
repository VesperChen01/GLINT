...
try:
    ...
    print("[GLINT] ✅ RDKit loaded")
except ImportError:
    print("[GLINT] ⚠️ RDKit not installed; some advanced features unavailable")
    print("[GLINT] 💡 Install manually: pip install rdkit")

try:
    import numpy as np
    NUMPY_AVAILABLE = True
except ImportError:
    print("[GLINT] ⚠️ NumPy not installed")
...
try:
    ...
    print("[GLINT] ✅ Using refactored interaction detection (performance boost)")
...
def is_cationpi(res1, atoms1, res2, atoms2):
    ...
    if cat1 and ring2:
        c1, n1 = cat1
        r2_coords, n2 = ring2
        d = distance(c1, centroid(r2_coords))
        if d <= MAX_CATION_PI_DIST:
    ...
    if cat2 and ring1:
        c2, n2 = cat2
        r1_coords, n1 = ring1
        d = distance(c2, centroid(r1_coords))
        if d <= MAX_CATION_PI_DIST:
        ...
        for prot_key, prot_atoms in protein_residues:
            ...
            if cationpi_res:
                ...
                def fmt_cationpi(p):
                    ...
                    if any(name in CATION_ATOM_SET for name in p):
                        return f"Cation({','.join(p)})"
                    else:
                        return f"Ring({','.join(p)})"
                ...
                interactions.append({
                    "Ligand_Chain": lig_chain,
                    "Ligand_Residue": f"{lig_name} {lig_id}",
                    "Ligand_Atom": fmt_cationpi(lig_part),
                    "Ligand_Atom_X": round(lig_cen[0], 3),
                    "Ligand_Atom_Y": round(lig_cen[1], 3),
                    "Ligand_Atom_Z": round(lig_cen[2], 3),
                    "Protein_Chain": prot_chain,
...
def visualize_protein_ligand_3d(obj_name, interactions_result=None, ligand_resname=None,
    ...
    max_interactions_per_type = settings.max_interactions_per_type
    from pymol import cmd
            ...
            try:
                ...
                if "ring" in str(lig_atom).lower() or "ring" in str(prot_atom).lower() or "cation" in str(lig_atom).lower() or "cation" in str(prot_atom).lower():
                    try:
                        ...
                        def get_cation_center(obj, chain, resi, atom_str):
                            """从 Cation(NZ,...) 格式中提取阳离子原子坐标中心"""
                            import re as _re
                            m = _re.search(r'[Cc]ation\(([^)]+)\)', str(atom_str))
                            if m:
                            ...
                            if atom_names:
                                ...
                                for aname in atom_names:
                                    if chain and chain.strip():
                                        s = f"{obj} and chain {chain} and resi {resi} and name {aname}"
                                    else:
                                        s = f"{obj} and resi {resi} and name {aname}"
                                    try:
                                        mdl = cmd.get_model(s)
                                        if mdl.atom:
                                            c = mdl.atom[0].coord
                            ...
                            if chain and chain.strip():
                                ...
                                base_sel = f"{obj} and resi {resi}"
                            try:
                                mdl = cmd.get_model(base_sel)
                                if mdl.atom:
                                    cation_coords = []
                                    for a in mdl.atom:
                                        if a.name.strip() in KNOWN_CATION_ATOMS:
                                            cation_coords.append((a.coord[0], a.coord[1], a.coord[2]))
                                            print(f"[get_cation_center] DEBUG fallback: 找到阳离子原子 {a.name.strip()} at ({a.coord[0]:.2f}, {a.coord[1]:.2f}, {a.coord[2]:.2f})")
                                    if cation_coords:
                                        x = sum(c[0] for c in cation_coords) / len(cation_coords)
                                        y = sum(c[1] for c in cation_coords) / len(cation_coords)
                        ...
                        # 判断配体/蛋白质侧是环还是阳离子
                        lig_is_cation = "cation" in str(lig_atom).lower()
                        prot_is_cation = "cation" in str(prot_atom).lower()

                        # 计算配体侧中心
                        if lig_is_cation:
                            lig_center = get_cation_center(obj_name, lig_chain, lig_resid, lig_atom)
                            print(f"[visualize_protein_ligand_3d] DEBUG: 配体是阳离子, center={lig_center}")
                        else:
                            lig_center = get_ring_center_from_selection(obj_name, lig_chain, lig_resid, lig_resname_int)

                        # 计算蛋白质侧中心
                        if prot_is_cation:
                            prot_center = get_cation_center(obj_name, prot_chain, prot_resid, prot_atom)
                            print(f"[visualize_protein_ligand_3d] DEBUG: 蛋白质是阳离子, center={prot_center}")
                        else:
