...
Thank you for your support! 🚀
    ...
    def check_environment(self):
        ...
        try:
            ...
            reply = QMessageBox.question(
                ...
                "This will:\n"
                "  1. Create conda environment 'gluetk'\n"
    ...
    def _run_install_script(self):
        ...
        try:
            import gluetk
            gluetk_dir = os.path.dirname(os.path.abspath(gluetk.__file__))
            candidates.append(os.path.join(os.path.dirname(gluetk_dir), "install.sh"))
        except:
            pass
        ...
        # 生成内联安装命令（当找不到脚本时使用）
        inline_cmd = '''echo "============================================================"
echo "🧬 GlueTK - Installing Dependencies"
echo "============================================================"
conda create -n gluetk python=3.9 -y
conda install -n gluetk -c conda-forge rdkit scipy matplotlib pillow numpy pandas seaborn pyqt openbabel pymol-open-source boost boost-cpp swig -y
...
echo ""
echo "🚀 To start GlueTK:"
echo "   conda activate gluetk && pymol"
echo "============================================================"'''
        ...
        try:
            if sys.platform == "darwin":  # macOS
                ...
                # Windows: 写入临时批处理文件
                import tempfile
                bat_file = os.path.join(tempfile.gettempdir(), "gluetk_install.bat")
            ...
            QMessageBox.information(
                ...
                "After it completes:\n"
                "  1. Close this PyMOL\n"
                "  2. Run: conda activate gluetk && pymol"
            )
            ...
            QMessageBox.warning(
                ...
                "  conda create -n gluetk python=3.9 -y\n"
                "  conda install -n gluetk -c conda-forge rdkit scipy "
...
# ============================================================================
# GlueTKDialog continued - Style and Scaling Methods
# (These belong to GlueTKDialog class, not DiseaseAnalysisTab)
# ============================================================================

# Monkey-patch these methods onto GlueTKDialog
def _gluetk_setup_style(self):
...
# -------- 将 setup_style 绑定到 GlueTKDialog --------
GlueTKDialog.setup_style = _gluetk_setup_style
# -------- 独立运行入口（可选）--------
