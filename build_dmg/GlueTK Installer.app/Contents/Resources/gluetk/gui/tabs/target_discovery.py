...
from ..workers import GMotifWorker, C2H2Worker, SurfaceAnalysisWorker, SurfaceSimilarityWorker
...
class TargetDiscoveryTab(CommonTab):
    ...
    def init_ui(self):
        ...
        self.parent_window.gm_btn_surface = QPushButton("Show Surface"); self.parent_window.gm_btn_surface.setObjectName("highlight_btn")
        self.parent_window.gm_btn_surface.clicked.connect(self.show_gloop_surface)
        ...
        self.parent_window.gm_btn_surf_analysis = QPushButton("Surface Analysis"); self.parent_window.gm_btn_surf_analysis.setObjectName("primary_btn")
        self.parent_window.gm_btn_surf_analysis.clicked.connect(self.analyze_gloop_surface)
        self.parent_window.gm_btn_surf_analysis.setToolTip("Analyze electrostatic and hydrophobic patches around G-loop")
        gm_btn_row.addWidget(self.parent_window.gm_btn)
        gm_btn_row.addWidget(self.parent_window.gm_btn_surface); gm_btn_row.addWidget(self.parent_window.gm_btn_coords)
        gm_btn_row.addWidget(self.parent_window.gm_btn_surf_analysis)
        ...
        grp_c2h2 = QGroupBox("C2H2 Zinc Finger Detection (IKZF1/3, ZNF, etc.)")
        ...
        # Row 0: Description/Help
        c2h2_help = QLabel("Detects C2H2 zinc finger domains (e.g., IKZF1/3 for lenalidomide targets). "
                          "Load a protein structure, then click 'Find Zinc Fingers'.")
        c2h2_help.setStyleSheet("color: #888; font-style: italic;")
        c2h2_help.setWordWrap(True)
        c2h2_grid.addWidget(c2h2_help, 0, 0, 1, 4)
        
        # Row 1: Target Object / PDB File
        c2h2_grid.addWidget(QLabel("Target Object:"), 1, 0, Qt.AlignmentFlag.AlignRight)
        ...
        r1_c2h2 = QHBoxLayout(); r1_c2h2.addWidget(self.parent_window.obj_combo_c2h2, 1); r1_c2h2.addWidget(self.parent_window.refresh_obj_c2h2)
        c2h2_grid.addLayout(r1_c2h2, 1, 1)
        
        c2h2_grid.addWidget(QLabel("PDB File (opt):"), 1, 2, Qt.AlignmentFlag.AlignRight)
        ...
        r1b_c2h2 = QHBoxLayout(); r1b_c2h2.addWidget(self.parent_window.c2h2_pdb, 1); r1b_c2h2.addWidget(self.parent_window.c2h2_pdb_browse)
        c2h2_grid.addLayout(r1b_c2h2, 1, 3)
        
        # Row 2: Turn RMSD / Global RMSD
        c2h2_grid.addWidget(QLabel("Turn RMSD (Å):"), 2, 0, Qt.AlignmentFlag.AlignRight)
        self.parent_window.c2h2_turn_rmsd = QLineEdit("2.0")
        self.parent_window.c2h2_turn_rmsd.setToolTip("Local turn alignment RMSD threshold (more sensitive)")
        c2h2_grid.addWidget(self.parent_window.c2h2_turn_rmsd, 2, 1)
        
        c2h2_grid.addWidget(QLabel("Global RMSD (Å):"), 2, 2, Qt.AlignmentFlag.AlignRight)
        self.parent_window.c2h2_global_rmsd = QLineEdit("3.5")
        self.parent_window.c2h2_global_rmsd.setToolTip("Global fold check RMSD threshold")
        c2h2_grid.addWidget(self.parent_window.c2h2_global_rmsd, 2, 3)
        
        # Row 3: Checkboxes
        self.parent_window.c2h2_require_turn_gly = QCheckBox("Require Turn Gly")
        self.parent_window.c2h2_require_turn_gly.setChecked(False)  # Default: no hard filter
        self.parent_window.c2h2_require_turn_gly.setToolTip("Require key Gly in turn region (enabling reduces recall)")
        c2h2_grid.addWidget(self.parent_window.c2h2_require_turn_gly, 3, 1)
        
        self.parent_window.c2h2_skip_low_complexity = QCheckBox("Skip Low-Complexity")
        self.parent_window.c2h2_skip_low_complexity.setChecked(False)  # Default: keep but downweight
        self.parent_window.c2h2_skip_low_complexity.setToolTip("Skip polyQ and other low-complexity regions (enabling reduces recall)")
        c2h2_grid.addWidget(self.parent_window.c2h2_skip_low_complexity, 3, 3)
        
        # Row 4: Output CSV
        c2h2_grid.addWidget(QLabel("Output CSV:"), 4, 0, Qt.AlignmentFlag.AlignRight)
        ...
        r4_c2h2 = QHBoxLayout(); r4_c2h2.addWidget(self.parent_window.c2h2_out_csv, 1); r4_c2h2.addWidget(self.parent_window.c2h2_out_browse)
        c2h2_grid.addLayout(r4_c2h2, 4, 1, 1, 3)
        
        # C2H2 Buttons
        c2h2_btn_row = QHBoxLayout()
        self.parent_window.c2h2_btn = QPushButton("Find Zinc Fingers"); self.parent_window.c2h2_btn.setObjectName("primary_btn")
        self.parent_window.c2h2_btn.clicked.connect(self.start_c2h2)
        self.parent_window.c2h2_btn.setToolTip("Detect C2H2 zinc finger domains in the loaded structure")
        ...
        self.parent_window.c2h2_btn_render.setToolTip("Visualize detected zinc fingers with electrostatic surface")
        self.parent_window.c2h2_btn_highlight = QPushButton("Highlight Domains"); self.parent_window.c2h2_btn_highlight.setObjectName("highlight_btn")
        self.parent_window.c2h2_btn_highlight.clicked.connect(self.highlight_c2h2_domains)
        self.parent_window.c2h2_btn_highlight.setToolTip("Highlight detected zinc finger domains in PyMOL")
        c2h2_btn_row.addWidget(self.parent_window.c2h2_btn); c2h2_btn_row.addWidget(self.parent_window.c2h2_btn_render)
        c2h2_btn_row.addWidget(self.parent_window.c2h2_btn_highlight); c2h2_btn_row.addStretch(1)
    ...
    def start_gmotif(self):
        ...
        # Template options: 0=GSPT1, 1=CK1α, 2=VAV1, 3=From Selection
        if idx in (0, 1, 2):
            template_mode = "builtin"
            template_sel = None
            # Use simplified template names (consistent with BUILTIN_TEMPLATES in g_motif_analyzer.py)
            template_builtin = ["GSPT1", "CK1α", "VAV1"][idx]
        else:
            # idx == 3: From Selection
    ...
    def _visualize_gloop_patches(self, obj_name: str, patches: list):
        ...
        try:
            ...
            for p in patches:
                ...
                patch_sel = f"patch_{p.id}_{p.type[:4]}"
                
                cmd.select(patch_sel, f"{obj_name} and ({sel_str})")
                
                # Color based on type
                if p.type == "electrostatic_pos":
                    cmd.color("red", patch_sel)
                elif p.type == "electrostatic_neg":
                    cmd.color("blue", patch_sel)
                elif p.type == "hydrophobic":
                    cmd.color("green", patch_sel)
                
                # Reduce transparency for patches
                cmd.set("transparency", 0.2, patch_sel)
    ...
    def highlight_c2h2_domains(self):
        """Highlight detected C2H2 zinc finger domains in PyMOL"""
        if not self._c2h2_hits:
            QMessageBox.warning(self, "Warning", "No C2H2 domains found. Please run detection first.")
            return
        ...
        try:
            ...
            # Highlight each C2H2 domain
            colors = ["cyan", "magenta", "yellow", "orange", "green", "pink", "blue", "red"]
            
            for i, hit in enumerate(self._c2h2_hits):
                chain, seq, start, end, turn_rmsd, score, status = hit
                
                # Create selection for this domain
                sel_name = f"c2h2_{i+1}"
                sel_str = f"{obj} and chain {chain} and resi {start}-{end}"
                ...
                if status == "pass":
                    ...
                    color = "white"  # Candidate domains in white
                
                cmd.color(color, sel_name)
                cmd.show("sticks", sel_name)
                cmd.set("stick_radius", 0.15, sel_name)
                
                # Add label
                label_sel = f"{sel_name} and name CA and resi {start}"
                cmd.label(label_sel, f"'ZF{i+1}'")
                
                self.log(f"✅ Highlighted C2H2 domain {i+1}: {chain}:{start}-{end} ({status})")
            
            # Zoom to all domains
            if self._c2h2_hits:
                all_sel = " or ".join([f"c2h2_{i+1}" for i in range(len(self._c2h2_hits))])
                cmd.zoom(all_sel, buffer=8.0)
            
            self.log(f"   Total: {len(self._c2h2_hits)} zinc finger domains highlighted")
            self.log("   Colors: pass=colored, candidate=white")
            
        except Exception as e:
            self.on_error(f"C2H2 highlight failed: {e}")
